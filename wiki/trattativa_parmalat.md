---
tipo: contratto
fonte: email
confidenza: alta
aggiornato: 2026-05-14
---

# Trattativa — Parmalat

## Stato: Call positiva 11/05 — LOI in preparazione

| Campo | Valore |
|-------|--------|
| Contatto Parmalat | **Chiara Canedoli** — Director Marketing |
| Intermediario | **Silvia Napolitano** (FLU) |
| Meeting pitch | 16 Febbraio 2026 |
| Account ND | Fabio Berra |
| Ultimo aggiornamento | 15 Febbraio 2026 |

---

## Proposta

Attivazione **Conversational Signage** in-store Parmalat su 3 brand:
- **Zymil** — Quiz Benessere Interattivo
- **Chef** — Ricettario Interattivo Stagionale
- **Santal** — Fruit Matcher Game

**Formato pilota**: 10-15 PV, 8 settimane

---

## Materiali Prodotti

| File | Stato |
|------|-------|
| `Pitch_Parmalat_Designed.pptx` (ultima versione 16:03 del 15/02) | Pronto |
| `parmalat_slides_content.json` — struttura 45 slide | Pronto |
| Diagrammi Nanobanan (3 pilastri, data flow, KPI) | Generati |
| Mockup totem brandizzati | Generati |
| `FLU_CompanyProfile_Parmalat.pptx` (da FLU, 144 MB) | Ricevuto |

Tutti i file in: `JarvisNEXT/materiale/pitch-parmalat/`

---

## Flow Narrativo (SCR Framework)

Chi Siamo → Situation (20M famiglie, multi-brand) → Complication (PdV territorio cieco) → Resolution (Conversational Signage) → Attivazioni brand → Case Study → Pilota → Next Steps

---

## Aggiornamento — 07 Maggio 2026

Meeting interno di preparazione (PLAUD 07/05/2026 ore 10:40):
- **Retail media per Parmalat**: approfondimento strategia campagne
- **Testate di gondola**: focus su posizionamento fisico in-store (testata = estremità dello scaffale — alta visibilità)
- **KPI di vendita**: definizione metriche sell-out e performance
- **Orchestrazione dati**: integrazione layer dati per misurazione impatto

## Aggiornamento — 11 Maggio 2026 ★★★

**Mattina**: revisione presentazione e offerta (PLAUD tuning session ore 09:46)

**Call con Parmalat andata bene** (Fabio Berra, ore ~14:00):
> "La call con Parmalat è andata bene e ha dato disponibilità a creare un doc che vada nella
> direzione della LOI in cui elenchiamo i requisiti che deve avere una soluzione retail media."

**Risposta Alessio Di Domizio (Durante)**:
> "Ottima notizia. Vogliamo standardizzare internamente i requirements così che diventino
> contemporaneamente parte delle presentazioni e delle LOI che auspicabilmente ne [seguono]."

→ **Parmalat ha dato verde alla LOI.** Durante vuole standardizzare i requirements.

---

## Aggiornamento — 13/05/2026

**Meeting Parmalat + Retail media Conad confermato 15/05/2026 ore 15:00-16:00 (Microsoft Teams)**:
- Fabio Berra ha accettato l'invito
- Focus: retail media in Conad

**Follow-up presentazione 11/05** — Fabio Berra → Alessio Di Domizio (Durante):
- Accordo per lavorare a 4 mani sulle slides con gli spunti condivisi da Alessio con Marco
- Check proposto per pomeriggio 14/05
- Team: Fabio Berra (ND), Alessio Di Domizio (Durante), Matteo Cozzi, Annachiara Orlandini, Matteo Sangalli

---

## Next Steps Aperti

- [x] ~~Raccogliere feedback post-meeting FLU/Parmalat~~ → Call andata bene 11/05
- [x] ~~Standardizzare requirements con Durante~~ → Alessio Di Domizio e Fabio Berra al lavoro sulle slides
- [ ] **Check slides Fabio + Alessio** — 14/05 pomeriggio
- [ ] **Meeting Parmalat Retail media Conad — 15/05/2026 ore 15:00** (Teams)
- [ ] **Creare documento requirements** per soluzione retail media (base per presentazioni + LOI)
- [ ] Redigere LOI formale
- [ ] Mockup totem Zymil/Chef/Santal (se richiesti)
- [ ] Definire orchestrazione dati per misurazione campagne

---

## Fonte
`JarvisNEXT/materiale/pitch-parmalat/STATUS_PARMALAT_PITCH.md`
`NDU/raw/2026-05-07_10-40_plaud-parmalat-retail-media-KPI.md`
`NDU/raw/2026-05-11_parmalat-call-loi.md`
`NDU/raw/2026-05-11_plaud-parmalat-presentazione.md`
